...
Windows desktop automation uses Codex's installed Computer Use plugin through the configured Node REPL MCP channel. The bridge does not implement its own mouse or keyboard injector. The current official Computer Use runtime is Windows-specific; macOS desktop control remains unavailable until an equivalent approval-preserving runtime exists.
