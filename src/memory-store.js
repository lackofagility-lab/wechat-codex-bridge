...
export class MemoryStore {
  ...
  buildPrompt(userId, userText) {
    ...
    return [
      ...
      ...(this.computerUseEnabled ? ["如果用户要求查看或操作 Windows 桌面、应用、鼠标或键盘，请使用已安装的 computer-use 技能。用户说打开 Google、谷歌、Chrome 或浏览器时，视为要求通过 Computer Use 操作 Google Chrome。必须遵循该技能的安全确认规则：只读查看可直接执行；发送消息、删除内容、安装软件、提交表单、付款或其他外部副作用必须先在微信中请求明确确认。不要使用临时的 PowerShell/SendKeys 替代 Computer Use。近期记忆中的工具不可用或 sandboxCwd 错误只代表历史状态；收到新的桌面请求时必须先实际重试一次当前 Computer Use 通道，不能仅凭旧错误直接拒绝。" ] : []),
      "", "用户本轮消息：", userText,
    ].join("\n");
  }
